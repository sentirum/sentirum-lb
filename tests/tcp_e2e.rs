...
async fn tcp_plain_proxy_proto_and_tcp_sni_smoke() {
    ...
    let dynamic = spawn_plain_tcp_echo_backend().await;
    let dynamic_listen_port = free_port().await;
    let _dynamic_runtime = spawn_tcp_proxy(
        "tcp-dynamic",
        None,
        format!(
            "route add nats :{dynamic_listen_port} tcp://127.0.0.1:{backend} opts \"ssrfskipverify=true proto=tcp\"\n",
            backend = dynamic.addr.port(),
        ),
        None,
    )
    .await;
    wait_for_tcp_listener(dynamic_listen_port).await;
    let mut dynamic_client = TcpStream::connect(("127.0.0.1", dynamic_listen_port))
        .await
        .unwrap();
    dynamic_client.write_all(b"dyno").await.unwrap();
    let mut dynamic_echoed = [0_u8; 4];
    dynamic_client
        .read_exact(&mut dynamic_echoed)
        .await
        .unwrap();
    assert_eq!(&dynamic_echoed, b"dyno");
    ...
    let tcp = TcpStream::connect(("127.0.0.1", sni_listen_port))
        .await
        .unwrap();
    let server_name = ServerName::try_from("nats.example.com").unwrap().to_owned();
    ...
    let https_passthrough = spawn_tls_echo_backend("nats.example.com").await;
    ...
    let https_runtime = spawn_tcp_proxy(
        ...
        format!(
            "route add nats nats.example.com tcp://127.0.0.1:{backend} opts \"ssrfskipverify=true proto=tcp\"\n",
            backend = https_passthrough.addr.port(),
        ),
        Some((
            proxy_cert_pem.clone(),
            proxy_key_pem,
            format!("127.0.0.1:{public_tls_port}"),
        )),
    )
    .await;

    let mut passthrough_roots = RootCertStore::empty();
    for cert in load_certs(&https_passthrough.cert_pem) {
        passthrough_roots.add(cert).unwrap();
    }
    let passthrough_client_config = RustlsClientConfig::builder()
        .with_root_certificates(passthrough_roots)
        .with_no_client_auth();
    let passthrough_connector = TlsConnector::from(Arc::new(passthrough_client_config));
    let tcp = TcpStream::connect(("127.0.0.1", public_tls_port))
        .await
        .unwrap();
    let server_name = ServerName::try_from("nats.example.com").unwrap().to_owned();
    let mut tls = passthrough_connector
        .connect(server_name, tcp)
        .await
        .unwrap();
    tls.write_all(b"mux-sni!!").await.unwrap();
    let mut mux_echoed = [0_u8; 9];
    tls.read_exact(&mut mux_echoed).await.unwrap();
    assert_eq!(&mux_echoed, b"mux-sni!!");

    let https_client = reqwest::Client::builder()
        .use_rustls_tls()
        .add_root_certificate(Certificate::from_pem(proxy_cert_pem.as_bytes()).unwrap())
        .build()
        .unwrap();
    let resp = https_client
        .get(format!("https://localhost:{public_tls_port}/health"))
        .send()
        .await
        .unwrap();
...
async fn spawn_plain_tcp_echo_backend() -> EchoBackend {
    let listener = TcpListener::bind("127.0.0.1:0").await.unwrap();
    let addr = listener.local_addr().unwrap();
...
async fn wait_for_tcp_listener(port: u16) {
    ...
    panic!("tcp listener on port {port} did not become ready in time");
}
